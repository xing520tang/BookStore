...
public class CommonLog {
	...
	public void before(JoinPoint joinPoint) {  //传入JoinPoint用于获取方法名字等信息，且改参数需要放在第一位
		...
		System.out.println("AopLog>>>>[方法进] "+ joinPoint.getSignature().getName()+"(" + Arrays.asList(args) + ")");
	}
	...
	public void afterReturning(JoinPoint joinPoint, Object result) {
		...
		System.out.println("AopLog<<<<[方法出] "+ joinPoint.getSignature().getName() + "{" + result + "}");
	}
	...
	public void afterThrowing(JoinPoint joinPoint, Exception exp) {
		...
		System.out.println("AopLog<<<<[方法出] " + joinPoint.getSignature().getName() + "<" + exp + ">");
	}
}
